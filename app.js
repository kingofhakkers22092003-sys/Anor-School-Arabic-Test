...
async function refreshServerCache() {
  try {
    const [students, questions, mode] = await Promise.all([apiGet('/api/students'), apiGet('/api/questions'), apiGet('/api/grading-mode')]);
    saveStudents(students); saveQuestionBank(questions); localStorage.setItem(GRADING_MODE_STORE, mode.mode);
